...
def _export(f: Callable, args: Tuple[Value], constraints: List[Constraint] = []) -> torch.fx.GraphModule:
    """
    ...
    with torch._dynamo.config.patch(dataclasses.asdict(ExportDynamoConfig())):
        gm, _ = torch._dynamo.export(f, *args, aten_graph=True, tracing_mode="symbolic", decomposition_table=DECOMP_TABLE, constraints=constraints)
    return gm
